...
public sealed class ETagMiddleware(RequestDelegate next)
{
    ...
    public async Task InvokeAsync(HttpContext context, InMemoryETagStore inMemoryETagStore)
    {
        ...
        string? ifNoneMatch = context.Request.Headers.IfNoneMatch.FirstOrDefault()?.Replace("\"", string.Empty);
        string? ifMatch = context.Request.Headers.IfMatch.FirstOrDefault()?.Replace("\"", string.Empty);
